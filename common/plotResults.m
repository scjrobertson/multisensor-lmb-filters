function plotResults(model, measurements, groundTruth, stateEstimates, groundTruthRfs)
% PLOTRESULTS -- Plots the ground truth and filter state estimates
%   plotResults(model, groundTruth, stateEstimates, groundTruthRfs)
%
...
%% Stack measurements
measurementTimestamps = zeros(1, 0);
z = zeros(model.zDimension, 0);
for i = 1:simulationLength
    repeatedTimestamps = model.T * (i-1) * ones(1, numel(measurements{i}));
    measurementTimestamps = horzcat(measurementTimestamps, repeatedTimestamps);
    y = measurements{i};
    if (numel(y) > 0)
        z = horzcat(z, [y{:}]);
    end
...
for i = 1:length(groundTruth)
    projectedGroundTruth = model.C * groundTruth{i}(2:end, :);
...
% Plot measurements
plot(z(1, :), z(2, :), 'LineStyle', 'none', 'Marker', '+', 'MarkerSize', 2, 'Color', [0.5 0.5 0.5]);
% State estimates
...
% matlab2tikz([model.dataAssociationMethod 'XyPlot.tex']);
...
% Plot measurements
plot(measurementTimestamps, z(1, :), 'LineStyle', 'none', 'Marker', '+', 'MarkerSize', 2, 'Color', [0.5 0.5 0.5]);
% Plot simulation
...
% Plot measurements
plot(measurementTimestamps, z(2, :), 'LineStyle', 'none', 'Marker', '+', 'MarkerSize', 2, 'Color', [0.5 0.5 0.5]);
% Plot simulation
for i = 1:numberOfRfsTrajectories
    % Get saved data
    j = rfsTrajectories(i).lineIndex;
...
xlabel('t (s)');
ylabel('y (m)');
% matlab2tikz([model.dataAssociationMethod 'XyVTime.tex']);
